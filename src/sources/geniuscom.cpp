...
bool GeniusComSource::lookup(LyricDataRaw& data, abort_callback& abort)
{
    ...
    std::string url = "https://api.genius.com" + std::string(data.lookup_id) + "?text_format=plain";
    ...
    {   // Parser gets its own scope
        ...
        const cJSON* song_title = cJSON_GetObjectItem(song_song, "title");
        const cJSON* song_album = cJSON_GetObjectItem(song_song, "album");

        if (song_lyrics_plain == nullptr) { // fart
            LOG_WARN("Failed to download from genius.com page: No lyrics data!");
            cJSON_Delete(json);
            return {};
        }

        if (song_title->valuestring) { data.title = song_title->valuestring; };

        if (song_album->valuestring) {
            const cJSON* songAlbumName = cJSON_GetObjectItem(song_album, "name");
            const cJSON* songAlbumArtist = cJSON_GetObjectItem(song_album, "primary_artist_names");

            if (songAlbumName->valuestring) { data.title = songAlbumName->valuestring; };
            if (songAlbumArtist->valuestring) { data.title = songAlbumArtist->valuestring; };
        }
